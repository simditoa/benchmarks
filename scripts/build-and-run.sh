...
# Shared body of the itoa benchmark bootstrap. Runs from a checkout of this
# repo at the current working directory: installs build deps, probes
# /proc/cpuinfo for AVX-512 IFMA + VBMI, configures and builds the suite,
# emits a hardware-metadata header, and runs the benchmark.
#
...
#   EMIT_METADATA=1       # set to 0 to skip the hardware-metadata block
#   BENCH_MACHINE_TYPE=   # included verbatim in the metadata block

BUILD_TYPE="${BUILD_TYPE:-Release}"
EXTRA_CXX_FLAGS="${EXTRA_CXX_FLAGS:-}"
FORCE_AVX512="${FORCE_AVX512:-0}"
EMIT_METADATA="${EMIT_METADATA:-1}"
...
if [[ -z "${EXTRA_CXX_FLAGS}" ]]; then
  if [[ "${HAS_AVX512_IFMA}" -eq 1 && "${HAS_AVX512_VBMI}" -eq 1 ]] || [[ "${FORCE_AVX512}" -eq 1 ]]; then
    ...
    echo "    (set FORCE_AVX512=1 to override, or pass EXTRA_CXX_FLAGS explicitly)" >&2
  fi
else
